(function(view){
	...
	view.settings.broadcast.throttling.initPage = function() {
		EB_Common.validation.validate("addThrottlingForm",{
			rules: {
				name:{
					remote: {
						url:EB_Common.Ajax.wrapperUrl("/settings/broadcast/throttling/checkThrottleRuleName"),
						type:"post"
					}
				}
			},
    		...
    		submitHandler:function(form){
    			...
    			EB_Common.Ajax.post("/settings/broadcast/throttling",
    				{
		...
		EB_Common.validation.validate("updateThrottlingForm",{
    		submitHandler:function(form){
    			...
    			EB_Common.Ajax.put("/settings/broadcast/throttling",
    				{
    					id : tr.attr("id"),
    					name : newName,
    					rule:  newRule,
		...
		$('#formBut0').click(function() {
            ...
            EB_Common.Ajax.put("/settings/broadcast/throttling/flag?"+queryString,{},
        		function(data){ 
		...
		$("#code").change(function(){
			...
			$("#rule").rules("add", {
				remote: {
	    			url:EB_Common.Ajax.wrapperUrl("/settings/broadcast/throttling/checkThrottleRuleRule"),
	    			data:{
		...
		$('#editAmount').click(function(e){
			e.preventDefault();
			view.settings.broadcast.throttling.editAmount();
		});
		$('#saveAmount').click(function(e){
			e.preventDefault();
			view.settings.broadcast.throttling.saveUpdate();
		});
		$('#cancelAmount').click(function(e){
			e.preventDefault();
			view.settings.broadcast.throttling.cancelEdit();
		});
	...
	view.settings.broadcast.throttling.deleteType = function(obj) {
		...
		EB_Common.dialog.confirm(i18n['setting.delete.throttling'], i18n['global.dialog.title.confirm'], function() {
			$(this).dialog("close");
			EB_Common.Ajax.remove("/settings/broadcast/throttling", 
			{
				id : id
			}, function(data) {
				if (data == '-1') {
	...
	view.settings.broadcast.throttling.editAmount = function() {
		var span = $('#amountVal');
	...
	view.settings.broadcast.throttling.saveUpdate = function() {
		var amount = $("#newAmount").val();
		if ($("#newAmount").valid() == false){
			return;
		}
		EB_Common.Ajax.put("/settings/broadcast/throttling/defaultAmount", {
			amount : amount
		}, function(data) {
	...
	view.settings.broadcast.throttling.cancelEdit = function() {
		$("#amountForm").find("label.error").remove();
