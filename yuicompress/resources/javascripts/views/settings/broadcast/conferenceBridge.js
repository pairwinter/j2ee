	...
	view.settings.broadcast.conferenceBridge.initConferenceBridgePage = function() {
		EB_Common.validation.validate("addConferenceBridgeForm",{
			rules: {
				name:{
					remote: {
						url:EB_Common.Ajax.wrapperUrl("/settings/broadcast/conferenceBridge/checkConferenceBridgeName"),
						type:"post"
					}
				}
			},
    		...
    		submitHandler:function(){
    			...
    			var option = $("#code option[value='"
    					+ $("#code").val() + "']");
    			EB_Common.Ajax.post( "/settings/broadcast/conferenceBridge",
    				{
    				...
    				function(data) {
    					...
    					var str = '<tr id = "'+ data.conferenceBridge.id +'"><td>'
    							...
    							+ '</td><td>'
    							+ data.conferenceBridge.lastModifiedName
    							+ '</td><td>'
    							+ data.lastModifyDate
		...
		EB_Common.validation.validate("updateConferenceBridgeForm",{
    		submitHandler:function(){
    			...
    			EB_Common.Ajax.put("/settings/broadcast/conferenceBridge",
					{
						id : tr.attr("id"),
						name : newName,
						number : newNumber,
					...
					function(data) {
						...
						td4.attr("id","");
						tr.find("td").eq(4).text(data.conferenceBridge.lastModifiedName);
		...
		init_select();
		setIndex(0,0);
		var settings = {width:580,height:260,autoOpen:false,resizable: false, close:function() {
			clearDialog();
		}};
		...
		EB_Common.dialog.dialog("dialog",settings,function() {
			...
			if (codestr!=null){
				...
				if(codestr.indexOf("W") != 0){
					EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.wait"]);
					return;
				}
				if(codestr.indexOf("A") == -1){
					EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.code"]);
					return;
				}
		...
		$("#accesscodestr").keyup(function(e) {
			...
			if (!isDigits(accesscode)){
				EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.number"]);
...
//			console.log(codestr);
			...
			if(codestr.length > 50){
			    EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.length"]);
				return;
			}
			...
			if (id[1] == 'seconds') {
				...
				if (access==''){
					EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.empty"]);
					return false;
				}
				if(!isDigits(access)){
					EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.access"]);
				...
				if (j>=1){
					EB_Common.dialog.alert(i18n["setting.broadcast.conferenceBridges.error.moreCode"]);
					return false;
				}
				j++;
				i++;
	...
	view.settings.broadcast.conferenceBridge.deleteType = function(obj) {
		...
		EB_Common.dialog.confirm(i18n['setting.delete.conferenceBridge'], i18n['global.dialog.title.confirm'], function() {
			$(this).dialog("close");
			EB_Common.Ajax.remove("/settings/broadcast/conferenceBridge", {
				id : id
			}, function(data) {
				if (data == '-1') {
                    EB_Common.LeavePage.resetState();
	...
	view.settings.broadcast.conferenceBridge.changeCode = function(obj,accessPath){
		...
		for(var i=0;i<optionValues.length;i++){
			...
			if( type == "W" ){
				falg_i++;
				$('#codestr').append("<option value='W"+value+"'>"+index+". "+ i18n["setting.broadcast.conferenceBridges.select.wait"].replace(/\{(\d+)\}/g, value)+"</option>");	
			}
			if( type == "D" ){
				falg_i++;
				$('#codestr').append("<option value='D"+value+"'>"+index+". "+ i18n["setting.broadcast.conferenceBridges.select.dial"].replace(/\{(\d+)\}/g, value)+" </option>");
			}
			if( type == "A" ){
			    falg_i++;
				falg_j++;
				$('#codestr').append("<option value='A"+value+"''>"+index+". "+ i18n["setting.broadcast.conferenceBridges.select.code"].replace(/\{(\d+)\}/g, value)+"</option>");	
			}
		}
