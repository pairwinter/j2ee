(function(view){
	...
	view.settings.broadcast.senderInfo.initPage = function() {
		$('#formBut0').click(function() {
            ...
            EB_Common.Ajax.put("/settings/broadcast/senderInfo/permission?"+queryString,{},
            		function(data){ 
		...
		$("#addSenderInfo").click(function() {
			...
			EB_Common.Ajax.post("/settings/broadcast/senderInfo",
				{
					countryCode : code,
		...
		$('#editEmail').click(EB_View.settings.broadcast.senderInfo.editEmail);
		$('#saveEmail').click(EB_View.settings.broadcast.senderInfo.saveUpdate);
		$('#cancelEmail').click(EB_View.settings.broadcast.senderInfo.cancleEdit);
	};
	...
	function setDefault(e){
		...
		EB_Common.Ajax.put("/settings/broadcast/senderInfo/default", {
			id : tr.attr("id")
		}, function(data) {
			if (data == '-1') {
                EB_Common.LeavePage.resetState();
	...
	view.settings.broadcast.senderInfo.deleteType = function(obj) {
		...
		EB_Common.dialog.confirm(i18n['setting.delete.senderInfo'], i18n['global.dialog.title.confirm'], function() {
			$(this).dialog("close");
			EB_Common.Ajax.remove("/settings/broadcast/senderInfo", {
				id : id
			}, function(data) {
				if (data == '-1') {
                    EB_Common.LeavePage.resetState();
	...
	var email;
	view.settings.broadcast.senderInfo.editEmail = function(e) {
		$('#emailContent').hide();
	...
	view.settings.broadcast.senderInfo.saveUpdate = function(e) {
		if ($("#newEmail").valid() == false){
			return;
		}
		email = $("#newEmail").val();
		EB_Common.Ajax.put("/settings/broadcast/senderInfo/email", {
			emailFromDisplay : email
		}, function(data) {
	...
	view.settings.broadcast.senderInfo.cancleEdit = function(e) {
		$("#editEmail").closest('td').find("label.error").remove();
		$('#emailContent').text(email).show();
