(function(view) {
	...
	twitterthreshold.initPage = function() {
        ...
        var latDefault = 37.09024,
        	...
        	callbackFn = function(ct, data){
        		...
        		var text = ct == 'locateInMap' ? 'locationText' : 'locationTextPercentage';
		        twitterthreshold[text].selectGeoAddress = {
		        	country : data.country_name,
		        	formattedAddress : data.formatted_address,
		        	addressLine : data.address,// ? data.address.substring(0, data.address.lastIndexOf(',')) : '',
		        	latitude : data.latitude,
		        	longitude : data.longitude
		        };
        	    $('#' + text).val(data.formatted_address).valid();
        	};
		EB_Common.Ajax.ajax({
        ...
        $('#locateInMap,#locateInMap2').click(function(e){
        	e.preventDefault();
        	if($(this).hasClass('font-gray')){
        		return;
        	}
        	var id = this.id;
		    //count latitude and longitude
		    var selectGeoAddress,
		    	latitude,
		        longitude,
		        ct = id;
		    
		    if(id == 'locateInMap'){
		    	selectGeoAddress = twitterthreshold.locationText.selectGeoAddress;
		    }else{
		    	selectGeoAddress = twitterthreshold.locationTextPercentage.selectGeoAddress;
		    }
		        
	    	latitude = selectGeoAddress ? selectGeoAddress.latitude : 0 || latDefault;
	        longitude = selectGeoAddress ? selectGeoAddress.longitude : 0 || lngDefault;
		    var regions = {
		        latitude : latitude,
				longitude : longitude
		    };
	...
	function initDynamicFollowerTable(){
		...
		.jqGrid({
			autoencode:true,
			url : EB_Common.Ajax
					.wrapperUrl('/settings/iv/twitterThreshold/listDynamicFollowers'),
			height : 175,
			datatype : 'json',
			...
			colModel : [
					...
					},
					{
						...
						formatter : function(value,index,rowObject) {
							return '<a class="icn_trash_16" title="'+i18n['button.delete']+'" style="margin-right:32px;" onclick="return EB_View.twitterthreshold.removeDynamicFollower(\''+rowObject.screenName+'\');" href="javascript:void(0);"></a>';
						}
					}
	...
	function initTable() {
		twitterthreshold.twitterthresholdGrid
				.jqGrid({
					autoencode:true,
					url : EB_Common.Ajax
							.wrapperUrl('/settings/iv/twitterThreshold/listTwitterThresholds'),
					height : 175,
					datatype : 'json',
					emptyDataCaption : i18n['global.grid.emptyDataCaption'],
					autowidth : true,
					...
					colModel : [
							...
							},
							{
								...
								formatter : function(value) {
									return '<a class="icn_edit_16" title="'+i18n['button.edit']+'" onclick="EB_View.twitterthreshold.editTwitterThreshold('
											+ value
											+ ',this);" href="javascript:void(0);"></a>&nbsp;<a class="icn_trash_16" title="'+i18n['button.delete']+'" onclick="return EB_View.twitterthreshold.removeTwitterthreshold('
											+ value
	...
	twitterthreshold.renderLauchTemplatesColumn = function(value, options,rowObject) {
		...
		if(rowObject!=null){
			for ( var count = 0; count < rowObject.broadcasteTemplateTitle.length; count++) {
				broadcastTemplateContent += EB_Common.escapeHTML(rowObject.broadcasteTemplateTitle[count])
				+ "/";
			}
		}else{
			return '<label></label>';
	...
	twitterthreshold.renderStatusColumn = function(value, rec) {
		var startDiv = '';
		if (value == "Active") {
			startDiv += '<b class="b-grid-status" onclick="EB_View.twitterthreshold.formatUnit(this,'
					+ rec.rowId + ');"></b>';
		} else {
			startDiv += '<b class="b-grid-status off" onclick="EB_View.twitterthreshold.formatUnit(this,'
					+ rec.rowId + ');"></b>';
		}
		return startDiv;
	};
	...
	twitterthreshold.formatUnit = function(element, id) {
		var me = $(element);
		EB_Common.Ajax.post("/settings/iv/twitterThreshold/modifyStatus", {
	...
	twitterthreshold.editTwitterThreshold = function(value,elem) {
		EB_Common.Ajax.post("/settings/iv/twitterThreshold/changeThresholdStatusToInactive", {
			...
			if (data.status == "yes") {
				...
				EB_Common.Ajax.get("/settings/iv/twitterThreshold/getTwitterThreshold", {
					twitterThresholdId : value,
					time:new Date()
				}, function(data) {
					if (data.status == "yes") {
...
//						$('#twitterThresholdForm').valid();
	...
	twitterthreshold.removeTwitterthreshold = function(value) {
		EB_Common.dialog.confirm(
				...
				i18n['global.threshold.delete.comfirmtitle'], function() {
					$(this).dialog("close");
					EB_Common.Ajax.post("/settings/iv/twitterThreshold/removeTwitterThreshold", {
						twitterThresholdId : value,
						time:new Date()
					}, function(data) {
						if (data.status != "yes") {
	...
	twitterthreshold.removeDynamicFollower = function(value) {
		EB_Common.dialog.confirm(
				...
				i18n['global.threshold.delete.comfirmtitle'], function() {
					$(this).dialog("close");
					EB_Common.Ajax.post("/settings/iv/twitterThreshold/removeDynamicFollower", {
						screenName : value,
						time:new Date()
					}, function(data) {
						if (data.status != "yes") {
	...
	var addListeners = function() {
		...
		$('.toggleValidation').click(function(){
			...
			if(obj.attr('checked')){
				...
				if(obj.attr('id')=='radiusTextCheckbox'){
					...
					$('#locateInMap').addClass('font-gray');
				}else if(obj.attr('id')=='radiusTextPercentageCheckbox'){
					...
					$('#locateInMap2').addClass('font-gray');
		...
		$("#twitterThresholdForm").validate({
			...
			rules : {'thresholdName' : {
				remote:{
			          url:EB_Common.Ajax.wrapperUrl('/settings/iv/twitterThreshold/existsTwitterThreshold'),
			          type: "POST",
			          data: {
			        	  title: function() {
    ...
    twitterthreshold.validateTwitterUser=function(value,callback){
        ...
        $.ajax({
            ...
            url:EB_Common.Ajax.wrapperUrl('/settings/iv/twitterThreshold/getTwitterUser'),
            dataType: 'json',
            cache : false,
            ...
            success:function(data){
                if(data!=null&&data!=undefined){
					...
					if(data.user!=undefined){
						twitterthreshold.followName.fromFollower=data.user.id;
		                twitterthreshold.followName.twitterUserScreenName=data.user.screenName;
		                if(callback!=null&&callback!=undefined){
                        	callback('success');
                        }
						return;
	...
	twitterthreshold.getActiveAndInActiveThresholds = function() {
		EB_Common.Ajax.post("/settings/iv/twitterThreshold/getActiveAndInActiveThresholdNumber",{
	...
	twitterthreshold.getUsedAndRemainingThresholds = function() {
        EB_Common.Ajax.post("/settings/iv/twitterThreshold/getUsedAndRemainingThresholdNumber", function(data) {
            if (data.status != "yes") {
                EB_Common.dialog.alert(i18n[data.status],i18n['dialog.title.warning']);
            } else {
	...
	twitterthreshold.sendSave = function(threshold, region) {
		...
		if(threshold.fromUser!=undefined){
			delete threshold.fromUser;
		}
		EB_Common.Ajax.post('/settings/iv/twitterThreshold/saveTwitterThreshold',
						{
						...
						function(data) {
							if (data.status != "yes") {
								...
								twitterthreshold.twitterthresholdGrid.jqGrid("setGridParam",{
                                    url : EB_Common.Ajax.wrapperUrl("/settings/iv/twitterThreshold/listTwitterThresholds"),
                                    datatype : "json",
                                    emptyDataCaption : i18n['global.grid.emptyDataCaption'],
                                    mtype : 'get',
                                    loadError : function() {
                                        EB_Common.dialog.alert('load data error',i18n['dialog.title.warning']);
                                    },
                                    loadComplete:function(){
                                        twitterthreshold.cleanEditPanel();
	...
	twitterthreshold.cleanEditPanel = function() {
        ...
        twitterthreshold.locationText.closest('div').next().hide();
        twitterthreshold.locationTextPercentage.closest('div').next().hide();
	...
	twitterthreshold.setModel = function(threshold) {
		...
		if(threshold.denominatorField==null){
			...
			if(threshold.region!=null && threshold.region.geoAddress){
				...
				twitterthreshold.locationText.val(threshold.region.address);
		...
		if(threshold.sendingBroadcastEnabled){
			...
			if (selectBroadcastIds.length != 0){
				EB_Common.Ajax.post(
						"/settings/iv/twitterThreshold/searchSelectedBroadCastList",
						{
								...
								$.each( broadcastTemplates,
    										function(index, item) {
    											if(item!=null && item.message!=null){
    												broadCastTemplateContent += "<li><a href='javascript:void(0);' title='Remove' class='icn_trash_16' onclick='EB_View.twitterthreshold.removeBroadCastTemplate(this);' id='"
    													+ item.id
    													+ "'/>"
    													+ item.message.title
    													+ "</li>";
	...
	twitterthreshold.initBroadcastTemplate = function(e) {
		...
		if (title.length == 0)
			return;
		EB_Common.Ajax.get("/settings/iv/twitterThreshold/searchBroadCastList", {
                title : title,
                time:new Date()
            },
            function(data) {
                if (data.status != "yes") {
                    ...
                    $.each(broadcastTemplates,function(index, item) {
                        if(item!=null && item.message!=null){
                            ...
                            $('#broadcatTemplatesTw').append(bcTemplate) ;
                        }
