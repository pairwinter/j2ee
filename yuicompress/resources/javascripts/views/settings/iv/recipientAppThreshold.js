	...
	function initTable() {
		recipientAppThreshold.unsolicitedCategoryGrid
				.jqGrid({
					autoencode:true,
					url : EB_Common.Ajax
							.wrapperUrl('/settings/iv/recipientAppThreshold/listUnsolicitedCategorys'),
					height : 245,
					datatype : 'json',
					emptyDataCaption : i18n['global.grid.emptyDataCaption'],
					autowidth : true,
					colNames : [ i18n['recipientAppThreshold.field.category'],i18n['setting.iv.mobile.icon'], '' ],
					colModel : [
							{
								name : 'title',
								index : 'title',
							...
							},
							{
								...
								sortable : false,
								formatter : function(value, rec) {
									return '<a class="icn_edit_16" title="'+i18n['button.edit']+'" onclick="EB_View.recipientAppThreshold.editUnsolicitedCategory('
											+ value
											+ ',this);" href="javascript:void(0);"></a>&nbsp;<a class="icn_trash_16" title="'+i18n['button.delete']+'" onclick="return EB_View.recipientAppThreshold.removeUnsolicitedCategory('
											+ value
...
//					pager : "#unsolicitedCategoryGridPager",
	...
	recipientAppThreshold.editUnsolicitedCategory = function(value,obj) {
		...
		$('#addUnsolicitedCategoryForm').valid();
	};
		...
		EB_Common.dialog
				.confirm(
						...
						function() {
							...
							EB_Common.Ajax
									.post(
											"/settings/iv/recipientAppThreshold/removeUnsolicitedCategory",
											{
	...
	var addListeners = function() {
		...
		$('#addUnsolicitedCategoryForm').validate({
			rules : {'unsolicitedCategoryTextName' : {
							...
							maxlength:120,
							remote:{
						          url:EB_Common.Ajax.wrapperUrl("/settings/iv/recipientAppThreshold/existsUnsolicitedCategory"),   
						          type: "POST",
						          data: {                   
						        	  title: function() {
	...
	var saveUnsolicitedCategory = function() {
	...
	recipientAppThreshold.sendsaveUnsolicitedCategory = function(
		...
		EB_Common.Ajax.post('/settings/iv/recipientAppThreshold/saveUnsolicitedCategory', {
			...
			if (data.status != "yes") {
				...
				recipientAppThreshold.unsolicitedCategoryGrid.jqGrid("setGridParam",
																	{
																		loadComplete:function(){
																			EB_Common.ToolPrompt.show('saveUnsolicitedCategory',i18n['glocal.savesuccess']);
																			recipientAppThreshold.clearUnsolicitedCategory();
																		}
																	}).trigger("reloadGrid");
			}
	...
	recipientAppThreshold.modifyShareMessageSetting = function(id){
		...
		EB_Common.Ajax.post("/settings/iv/recipientAppThreshold/modifyMessageAllowSetting", {
			column : column,
			status:status
		}, function(data) {
			...
			if (data.status != "yes") {
				EB_Common.dialog.alert(i18n['setting.iv.recipientAppThreshold.notAllowSendMessage'],i18n['dialog.title.warning']);
                $('#allowSendMessage').attr("checked",false);
            }
		}, "json");
	}
