/**
 * Settings Publishing Options CMAS/WEA Sender Agency Name
 * 
 */
(function(view) {
    ...
    var pub = {
        ...
        initPage: function() {
            // row template
            this.rowTemplate = '<tr id="{id}">\n\
                            <td>\n\
                                <a href="#" title="Down" name="operte_down" class="icn_down_16" ></a>\n\
                                <a href="#" title="Up" name="operte_up" class="icn_up_16" ></a>\n\
                                <span class="margin10-L txt_middle">{order}</span></td>\n\
                            <td>{value}</td>\n\
                            <td>\n\
                                ...
                                <a href="#" title="Delete" name="operte_delete" class="icn_trash_16"></a>\n\
                            </td>\n\
                        </tr>';
            this.hideDeleteBtn();
            this.resetSequence();
        ...
        initOperteEvent: function() {
            ...
            $('#eb_grid tbody').on('click', 'a[name="operte_edit"]', function(e) {
                ...
                rowEl.children().each(function(index,element){
                    ...
                    if(col.editable === true){
                        ...
                        $('<input type="text" name="' + col.name + '" style="width:370px;" maxlength="'+col.validRule.maxlength+'" class="width_percent94" value="' + val + '"/>').appendTo($el).rules('add', col.validRule);
                    }
                });
            });
            ...
            $('#eb_grid tbody').on('click', 'a[name="operte_save"]', function(e) {
                ...
                var el = $(this),
                    ...
                    deleteEl = rowEl.find('a[name="operte_delete"]'),
                    id = rowEl.find('input[name="id"]').val();
                ...
                EB_Common.Ajax.put(
                    "/settings/network/senderagencyname",
                    parmData,
                    function(data) {
                        if(data == 0){
                            EB_Common.LeavePage.resetState();
                        ...
                        if(data == -1){
                            EB_Common.dialog.alert(i18n["setting.error.agencyName.duplicatedName"]);
                            return;
                        }
                        el.hide();
                        editEl.show();
            ...
            $('#eb_grid tbody').on('click', 'a[name="operte_up"]', function(e) {
                ...
                EB_Common.Ajax.put(
                    "/settings/network/senderagencyname/move",
                    parmData,
                    function(data) {
                        if(data == 0){
                            EB_Common.LeavePage.resetState();
            ...
            $('#eb_grid tbody').on('change', 'input[name="isDefault"]', function(e) {
                $.ajax({
                    url: '',
	...
	view.settings.network.senderAgencyName = pub;
