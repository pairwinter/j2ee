				...
				function(data) {
		    		...
		    		for(var i = 0; i < length; i++){
						...
						if(notificationVo.isEmpty){
							...
							try{
								loadchart(key.toString()+'_bc',notificationVo.type == "Polling" || notificationVo.type == "Quota" ? notificationVo.data : convertI18n(notificationVo.data));
							}catch(err){
								EB_Common.noData(key.toString()+"_bc");
							}
						}
		    		...
		    		if(size > 3){
		        		var more = $('<a class="margin10_B" href="'+EB_Common.Ajax.wrapperUrl("/notifications#ui-tabs-1")+'">'+i18n['dashboard.more']+'</a>');
