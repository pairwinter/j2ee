...
(function($){
	...
	data = {
		...
		//set default settings Linder Wang add
		/*
         $.fn.zTree._z.data.setDefaults(
		 	{
				removeTitle: "remove",
				renameTitle: "rename"
            }
		...
		*/
	    setDefaults : function(local){
	        if(!local){
	        	return;
	        }
	        if(local.removeTitle){
	        	_setting.edit.removeTitle = local.removeTitle;
	        }
	        if(local.renameTitle){
	        	_setting.edit.renameTitle = local.renameTitle;
	        }
	    },
	...
	_view = {
		...
		cancelCurEditNode: function (setting, forceName) {
			...
			if (node) {
				...
				if(zTree.addFlag === true){
					zTree.removeNode(node);
					zTree.addFlag = false;
				}
			}
			root.noSelection = true;
			tools.apply(setting.callback.afterCancel, [setting.treeId, node], true);
			return true;
		},
		editNode: function(setting, node) {
			...
			inputObj.attr({
				value : node[nameKey],
				maxlength : consts.editor.MAX_LENGTH
			});
			...
			/*.bind('keydown', function(event) {
				if (event.keyCode=="13") {
				    ...
				    if(event.keyCode != 8 && event.keyCode != 46 && $(this).val().length >= consts.editor.MAX_LENGTH){
				        $(this).val($(this).val().substring(0,consts.editor.MAX_LENGTH));
